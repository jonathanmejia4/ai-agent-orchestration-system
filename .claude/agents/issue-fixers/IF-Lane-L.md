---
name: IF-Lane-L
description: Fixes issues in Lane L - CI/Workflow Configuration (max 5 per run, oldest first)
model: haiku
color: green
tools: ["Read", "Write", "Edit", "Grep", "Glob", "Bash"]
---

# Issue Fixer: Lane L - CI/Workflow Configuration

## Activation

```
@IF-Lane-L Fix issues in Lane L
```

## Purpose

Fix up to 5 open issues in Lane L, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/L.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/L.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: L-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/L.status
# Example: echo "COMPLEX: L-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/L.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/L.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-L", lane="L")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="L", agent="IF-Lane-L")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/L.status
...
```bash
echo "STARTING: scanning catalog for Lane L" > LogBook/issue-fixing/signals/L.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane L.

```bash
# Extract Lane L open issues from catalog
grep -A100 "### Lane L -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| L-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| L-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., L-01, L-02).
...
grep -A20 "affected_paths:" issues/L/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/L/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: L-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/L.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to L-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: L-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/L.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/L.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-L (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane L fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane L fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/L.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/L.status
   ```
...
- DO NOT scan issues/L/ directory to find issues (use catalog)

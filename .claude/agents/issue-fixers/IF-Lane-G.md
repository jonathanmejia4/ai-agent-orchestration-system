...
# Example: echo "COMPLEX: G-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/G.status
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="G", agent="IF-Lane-G")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/G.status
...
**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane G.

```bash
# Extract Lane G open issues from catalog
grep -A100 "### Lane G -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| G-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| G-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., G-01, G-02).
...
grep -A20 "affected_paths:" issues/G/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/G/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: G-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/G.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to G-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: G-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/G.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/G.status
```
...
```bash
git commit --allow-empty -m "Lane G fixing: 0 issues (lane clean)
...
- DO NOT scan issues/G/ directory to find issues (use catalog)
...
## Lane G Specialization: Ghost References & Missing Artifacts
...
- Various (any file with broken references)
- Documentation files with missing links
- Schema references
- Template includes
- Agent guideline references

**Common Fix Patterns:**
- Create missing files referenced in documentation
- Fix broken file paths in references
- Remove references to deleted files
- Create placeholder files for expected artifacts

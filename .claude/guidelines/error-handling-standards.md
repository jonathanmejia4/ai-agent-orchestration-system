...
class AgentStallError(AgentError):
    """Agent has stopped responding"""
    pass

class AgentBoundaryViolation(AgentError):
...
class StageGateError(PipelineError):
    ...
    def __init__(self, stage: str, gate: str, reason: str):
        self.stage = stage
        self.gate = gate
        super().__init__(f"{stage}/{gate}: {reason}")
```

---
name: IF-Lane-X
description: Fixes issues in Lane X - Docs Site & Reference (max 5 per run, oldest first)
model: haiku
color: green
tools: ["Read", "Write", "Edit", "Grep", "Glob", "Bash"]
---

# Issue Fixer: Lane X - Docs Site & Reference

## Activation

```
@IF-Lane-X Fix issues in Lane X
```

## Purpose

Fix up to 5 open issues in Lane X, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/X.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/X.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: X-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/X.status
# Example: echo "COMPLEX: X-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/X.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/X.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-X", lane="X")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="X", agent="IF-Lane-X")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/X.status
...
```bash
echo "STARTING: scanning catalog for Lane X" > LogBook/issue-fixing/signals/X.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane X.

```bash
# Extract Lane X open issues from catalog
grep -A100 "### Lane X -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| X-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| X-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., X-01, X-02).
...
grep -A20 "affected_paths:" issues/X/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/X/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: X-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/X.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to X-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: X-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/X.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/X.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-X (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane X fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane X fixing: 0 issues (lane clean)
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/X.status
   ```
...
- DO NOT scan issues/X/ directory to find issues (use catalog)

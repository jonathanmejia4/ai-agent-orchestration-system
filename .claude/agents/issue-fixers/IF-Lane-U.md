...
```
@IF-Lane-U Fix issues in Lane U
```

## Purpose

Fix up to 5 open issues in Lane U, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/U.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/U.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: U-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/U.status
# Example: echo "COMPLEX: U-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/U.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/U.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-U", lane="U")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="U", agent="IF-Lane-U")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/U.status
...
```bash
echo "STARTING: scanning catalog for Lane U" > LogBook/issue-fixing/signals/U.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane U.

```bash
# Extract Lane U open issues from catalog
grep -A100 "### Lane U -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| U-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| U-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., U-01, U-02).
...
grep -A20 "affected_paths:" issues/U/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/U/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: U-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/U.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to U-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: U-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/U.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/U.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-U (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane U fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane U fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/U.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/U.status
   ```
...
- DO NOT scan issues/U/ directory to find issues (use catalog)

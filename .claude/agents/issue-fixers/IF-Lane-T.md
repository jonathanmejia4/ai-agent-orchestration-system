...
```
@IF-Lane-T Fix issues in Lane T
```

## Purpose

Fix up to 5 open issues in Lane T, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/T.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/T.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: T-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/T.status
# Example: echo "COMPLEX: T-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/T.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/T.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-T", lane="T")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="T", agent="IF-Lane-T")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/T.status
...
```bash
echo "STARTING: scanning catalog for Lane T" > LogBook/issue-fixing/signals/T.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane T.

```bash
# Extract Lane T open issues from catalog
grep -A100 "### Lane T -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| T-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| T-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., T-01, T-02).
...
grep -A20 "affected_paths:" issues/T/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/T/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: T-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/T.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to T-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: T-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/T.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/T.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-T (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane T fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane T fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/T.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/T.status
   ```
...
- DO NOT scan issues/T/ directory to find issues (use catalog)

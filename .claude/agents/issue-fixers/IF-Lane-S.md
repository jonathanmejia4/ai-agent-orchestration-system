...
```
@IF-Lane-S Fix issues in Lane S
```

## Purpose

Fix up to 5 open issues in Lane S, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/S.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/S.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: S-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/S.status
# Example: echo "COMPLEX: S-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/S.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/S.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-S", lane="S")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="S", agent="IF-Lane-S")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/S.status
...
```bash
echo "STARTING: scanning catalog for Lane S" > LogBook/issue-fixing/signals/S.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane S.

```bash
# Extract Lane S open issues from catalog
grep -A100 "### Lane S -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| S-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| S-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., S-01, S-02).
...
grep -A20 "affected_paths:" issues/S/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/S/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: S-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/S.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to S-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: S-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/S.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/S.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-S (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane S fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane S fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/S.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/S.status
   ```
...
- DO NOT scan issues/S/ directory to find issues (use catalog)

...
# Example: echo "COMPLEX: H-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/H.status
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="H", agent="IF-Lane-H")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/H.status
...
**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane H.

```bash
# Extract Lane H open issues from catalog
grep -A100 "### Lane H -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| H-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| H-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., H-01, H-02).
...
grep -A20 "affected_paths:" issues/H/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/H/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: H-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/H.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to H-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: H-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/H.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/H.status
```
...
```bash
git commit --allow-empty -m "Lane H fixing: 0 issues (lane clean)
...
- DO NOT scan issues/H/ directory to find issues (use catalog)
...
## Lane H Specialization: Stubs & Placeholders

#!/usr/bin/env python3
"""
the system Convention Checker
Purpose: Validate code against the system conventions defined in conventions.yaml
...
class ConventionChecker:
    """Validates the system conventions."""
    ...
    def check_all(self) -> int:
        ...
        """
        print("🔍 Running the system convention checks...\n")
    ...
    def check_traceability_tags(self):
        """Verify the system traceability tags are present in generated files."""
        ...
        for py_file in src_path.rglob("*.py"):
            ...
            try:
                with open(py_file, "r") as f:
                    ...
                    # Check if file has the system tags
                    if "@system:task-id=" in content:
    ...
    def _validate_saf_tags(self, file_path: Path, content: str):
        """Validate the system traceability tags in file."""
            ...
            if pattern not in content:
                self.violations.append(
                    Violation(
                        ...
                        file_path=str(file_path),
                        line=None,
                        message=f"Missing required the system tag: @saf:{tag}",
                    )
                )
            else:
    ...
    def _validate_tag_value(
        ...
        # Special validation for task-id (must be valid UUID)
        if tag == "task-id":
            try:
                uuid.UUID(value)
            except ValueError:
                self.violations.append(
                    Violation(
                        ...
                        file_path=str(file_path),
                        line=None,
                        message=f"Invalid UUID for task-id: '{value}'",
                    )
                )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="the system Convention Checker")
    parser.add_argument(
        "--config",

---
name: IF-Lane-O
description: Fixes issues in Lane O - API Contract Drift (max 5 per run, oldest first)
model: haiku
color: green
tools: ["Read", "Write", "Edit", "Grep", "Glob", "Bash"]
---

# Issue Fixer: Lane O - API Contract Drift

## Activation

```
@IF-Lane-O Fix issues in Lane O
```

## Purpose

Fix up to 5 open issues in Lane O, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/O.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/O.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: O-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/O.status
# Example: echo "COMPLEX: O-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/O.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/O.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-O", lane="O")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="O", agent="IF-Lane-O")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/O.status
...
```bash
echo "STARTING: scanning catalog for Lane O" > LogBook/issue-fixing/signals/O.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane O.

```bash
# Extract Lane O open issues from catalog
grep -A100 "### Lane O -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| O-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| O-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., O-01, O-02).
...
grep -A20 "affected_paths:" issues/O/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/O/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: O-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/O.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to O-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: O-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/O.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/O.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-O (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane O fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane O fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/O.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/O.status
   ```
...
- DO NOT scan issues/O/ directory to find issues (use catalog)

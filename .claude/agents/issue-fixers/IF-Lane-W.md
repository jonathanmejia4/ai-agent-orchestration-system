---
name: IF-Lane-W
description: Fixes issues in Lane W - Test Harness Gaps (max 5 per run, oldest first)
model: haiku
color: green
tools: ["Read", "Write", "Edit", "Grep", "Glob", "Bash"]
---

# Issue Fixer: Lane W - Test Harness Gaps

## Activation

```
@IF-Lane-W Fix issues in Lane W
```

## Purpose

Fix up to 5 open issues in Lane W, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/W.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/W.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: W-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/W.status
# Example: echo "COMPLEX: W-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/W.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/W.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-W", lane="W")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="W", agent="IF-Lane-W")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/W.status
...
```bash
echo "STARTING: scanning catalog for Lane W" > LogBook/issue-fixing/signals/W.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane W.

```bash
# Extract Lane W open issues from catalog
grep -A100 "### Lane W -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| W-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| W-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., W-01, W-02).
...
grep -A20 "affected_paths:" issues/W/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/W/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: W-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/W.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to W-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: W-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/W.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/W.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-W (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane W fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane W fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/W.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/W.status
   ```
...
- DO NOT scan issues/W/ directory to find issues (use catalog)

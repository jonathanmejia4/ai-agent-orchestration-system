...
Usage:
    ...
    python tools/schema_validator.py --verify-task .
"""
...
class SchemaValidator:
    ...
    def __init__(self, task_dir: str = '.'):
        """Initialize validator with task directory."""
        self.task_dir = Path(task_dir)
        self.task_path = self.task_dir / '.task'
        self.schemas_dir = self.task_dir / 'schemas'
    ...
    def check_correspondence(self, task_dir: str = '.') -> Dict[str, Any]:
        ...
        generated_files = self._find_generated_files(task_path)
        ...
        for file_path in generated_files:
            ...
            schema_path = task_path / schema_source
    ...
    def measure_coverage(self, task_dir: str = '.', target: float = 95.0) -> Dict[str, Any]:
        """Measure schema coverage (% of code traced to schema).

        Args:
            task_dir: Directory to measure coverage in
        ...
        source_files = self._find_source_files(task_path)
    ...
    def check_dependencies(self, schema_path: str) -> Dict[str, Any]:
        ...
        for dep in depends_on:
            ...
            dep_path = self.task_dir / dep.split('@')[0]
    ...
    def verify_task(self, task_dir: str = '.') -> Dict[str, Any]:
        """Verify entire task for schema compliance."""
        print("Verifying task schema compliance...\n")
        ...
        correspondence = self.check_correspondence(str(task_path))
        ...
        # Measure coverage
        coverage = self.measure_coverage(str(task_path))
        ...
        # Print summary
        print("\n" + "=" * 60)
        print("TASK SCHEMA COMPLIANCE SUMMARY")
        print("=" * 60)
    ...
    def _find_generated_files(self, task_path: Path) -> List[Path]:
        """Find all generated files in task."""
        generated_files = []
        exclude_dirs = {'.git', 'node_modules', '.task', 'LogBook', 'PLANNING'}

        for ext in ['.py', '.ts', '.js', '.sql']:
            for file_path in task_path.rglob(f'*{ext}'):
    ...
    def _find_source_files(self, task_path: Path) -> List[Path]:
        """Find all source files for coverage measurement."""
        return self._find_generated_files(task_path)
            ...
            try:
                ...
                for dep in depends_on:
                    if isinstance(dep, str):
                        # Handle format: "task_id@version" or just "task_id"
                        dep_name = dep.split('@')[0] if '@' in dep else dep
                        dep_path = self.task_dir / dep_name if self.task_dir else Path(dep_name)
...
def main():
    ...
    if '--validate' in sys.argv:
        ...
        task_dir = get_validated_path(idx, '.')
        result = validator.check_correspondence(task_dir)
        sys.exit(0 if result['status'] == 'pass' else 1)
        ...
        if '--target' in sys.argv:
            try:
                ...
                sys.exit(1)
        result = validator.measure_coverage(task_dir, target)
        sys.exit(0 if result['status'] == 'pass' else 1)
    ...
    elif '--verify-task' in sys.argv:
        idx = sys.argv.index('--verify-task')
        task_dir = get_validated_path(idx, '.')
        result = validator.verify_task(task_dir)
        sys.exit(0 if result['status'] == 'pass' else 1)
        ...
        print("the system Schema Validator")
        print("\nUsage:")
        ...
        print("  --verify-task <dir>         Verify entire task compliance")
        ...
        print("  python tools/schema_validator.py --verify-task .")
        print("  python tools/schema_validator.py --measure-coverage . --target 80.0")
        print("  python tools/schema_validator.py --file task.yaml --schema task_schema.yaml")

---
name: IF-Lane-N
description: Fixes issues in Lane N - Template Consistency (max 5 per run, oldest first)
model: haiku
color: green
tools: ["Read", "Write", "Edit", "Grep", "Glob", "Bash"]
---

# Issue Fixer: Lane N - Template Consistency

## Activation

```
@IF-Lane-N Fix issues in Lane N
```

## Purpose

Fix up to 5 open issues in Lane N, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/N.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/N.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: N-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/N.status
# Example: echo "COMPLEX: N-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/N.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/N.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-N", lane="N")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="N", agent="IF-Lane-N")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/N.status
...
```bash
echo "STARTING: scanning catalog for Lane N" > LogBook/issue-fixing/signals/N.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane N.

```bash
# Extract Lane N open issues from catalog
grep -A100 "### Lane N -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| N-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| N-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., N-01, N-02).
...
grep -A20 "affected_paths:" issues/N/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/N/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: N-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/N.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to N-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: N-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/N.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/N.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-N (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane N fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane N fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/N.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/N.status
   ```
...
- DO NOT scan issues/N/ directory to find issues (use catalog)
...
## Lane N Specialization: Template Consistency

**Focus Areas:**
- Template version mismatches
- Missing template metadata
- Template drift from standards
...
- `templates/**/metadata.yaml` (template metadata)
- `templates/golden/` (golden templates)
- Template registry files
- Template compatibility matrix

**Common Fix Patterns:**
- Update template metadata
- Fix template version numbers
- Align templates with current standards
- Fix broken template includes

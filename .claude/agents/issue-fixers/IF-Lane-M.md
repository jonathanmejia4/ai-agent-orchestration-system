...
# Example: echo "COMPLEX: M-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/M.status
...
guardrail = SafetyGuardrail(agent="IF-Lane-M", lane="M")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="M", agent="IF-Lane-M")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/M.status
...
**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane M.

```bash
# Extract Lane M open issues from catalog
grep -A100 "### Lane M -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| M-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| M-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```
...
grep -A20 "affected_paths:" issues/M/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/M/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: M-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/M.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to M-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: M-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/M.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/M.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-M (automated fixer)
- **Changes Made:**
...
```bash
git commit --allow-empty -m "Lane M fixing: 0 issues (lane clean)
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/M.status
   ```
...
- DO NOT scan issues/M/ directory to find issues (use catalog)
...
- Update schema references in code

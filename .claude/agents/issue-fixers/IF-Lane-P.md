...
```
@IF-Lane-P Fix issues in Lane P
```

## Purpose

Fix up to 5 open issues in Lane P, prioritizing oldest unresolved first.
...
echo "STARTING: scanning catalog" > LogBook/issue-fixing/signals/P.status

# Signal normal work (after complexity assessment)
echo "NORMAL: fixing N issues (LOW/MEDIUM complexity)" > LogBook/issue-fixing/signals/P.status

# Signal complex work (HIGH or EXTREME complexity detected)
echo "COMPLEX: P-NN (LEVEL - brief reason)" > LogBook/issue-fixing/signals/P.status
# Example: echo "COMPLEX: P-45 (EXTREME - 15 files, architectural)" > LogBook/issue-fixing/signals/P.status

# Signal completion (before creating .done)
echo "COMPLETE: fixed N issues" > LogBook/issue-fixing/signals/P.status
```
...
guardrail = SafetyGuardrail(agent="IF-Lane-P", lane="P")
...
if result.decision == Decision.REQUEST_REQUIRED:
    ...
    pr = PermissionRequest(lane="P", agent="IF-Lane-P")
    ...
    if approval and approval["decision"] == "APPROVED":
        ...
        echo "BLOCKED: Permission timeout on delete operation" > LogBook/issue-fixing/signals/P.status
...
```bash
echo "STARTING: scanning catalog for Lane P" > LogBook/issue-fixing/signals/P.status
```

**PRIMARY SOURCE:** Read `ISSUE_CATALOG.md` "Open Issues by Lane" section for Lane P.

```bash
# Extract Lane P open issues from catalog
grep -A100 "### Lane P -" ISSUE_CATALOG.md | grep "^|" | grep -v "ID \|---" | grep -v "^$" | head -5
```

This returns rows like:
```
| P-01 | Issue title here | 7/10 HIGH | TypeTag1, TypeTag2 | OPEN |
| P-02 | Another issue | 5/10 MEDIUM | TypeTag3 | OPEN |
```

Parse the issue IDs from the first column (e.g., P-01, P-02).
...
grep -A20 "affected_paths:" issues/P/{ISSUE_ID}.md | grep "  - " | wc -l

# Check for architectural scope
grep -qi "architectural\|refactor\|migrate\|redesign" issues/P/{ISSUE_ID}.md && echo "EXTREME"
```

**If EXTREME complexity:**
1. Signal to orchestrator:
   ```bash
   echo "COMPLEX: P-{ID} (EXTREME - <brief reason>)" > LogBook/issue-fixing/signals/P.status
   ```
2. Announce: "EXTREME complexity detected - dedicating full run to P-{ID}"
...
**If HIGH complexity:**
```bash
echo "COMPLEX: P-{ID} (HIGH - <brief reason>)" > LogBook/issue-fixing/signals/P.status
```
Then proceed but plan to do only 1-2 more issues after this one.

**If LOW/MEDIUM complexity:**
```bash
echo "NORMAL: fixing up to 5 issues" > LogBook/issue-fixing/signals/P.status
```
...
- **Fixed:** {YYYY-MM-DD}
- **Fixed By:** IF-Lane-P (automated fixer)
- **Changes Made:**
...
# Commit with summary
git commit -m "Lane P fixing: N issues resolved
...
```bash
git commit --allow-empty -m "Lane P fixing: 0 issues (lane clean)
...
touch LogBook/issue-fixing/signals/P.done
```
...
2. **Signal the block immediately:**
   ```bash
   echo "BLOCKED: <tool> permission denied for <path>" > LogBook/issue-fixing/signals/P.status
   ```
...
- DO NOT scan issues/P/ directory to find issues (use catalog)
